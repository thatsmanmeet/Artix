...
def get_article_data():
    ...
    soup = BeautifulSoup(get_html_file(new_url), 'html.parser')
...
# Take an entry
link = customtkinter.CTkEntry(master=frame, width=600, height=25, state="normal",
                              placeholder_text="Enter an article's url to load")
link.grid(row=0, column=0, padx=10, pady=5, sticky="")

# Button
go_button = customtkinter.CTkButton(master=frame, text="GO", command=get_article_data)
...
articleText.insert("0.0", "Enter an article link in the above bar and press on go button to load the article. This "
                          "doesn't work as a search!")
